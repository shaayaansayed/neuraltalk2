...
function layer:updateOutput(input)
  ...
  if self.dim == 2 then
    ...
    self.output:resize(input:size(1)*self.n, input:size(2), input:size(3))
    for k=1,input:size(1) do
      local j = (k-1)*self.n+1
      self.output[{ {j,j+self.n-1} }] = input[{ {k,k}, {}, {} }]:expand(self.n, input:size(2), input:size(3)) -- copy over
    end
  end
