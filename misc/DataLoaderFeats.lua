...
function DataLoader:getBatch(opt)
  ...
  local feat_filename = path.join(self.feats_dir, path.join(split, self.batch_ix[split] .. '.t7'))
  local batch_feats = torch.load(feat_filename)  
  self.batch_ix[split] = self.batch_ix[split] + 1
  if wrapped then self:resetIterator(split) end

  local data = {}
  data.feats = batch_feats
